...
def resize_mask(mask, scale, padding):
    ...
    cord = np.array(np.where(mask == mask.max()))
    _, ix = np.unique(cord[2, :], return_index=True)
    ix = np.sort(ix)
    for i in ix:
        buffer_mask[cord[0, i], cord[1, i], cord[2, i]] = 1
    return buffer_mask
...
def resize_keypoints(mask, new_size, scale, padding):
    ...
    for i in range(14):
        ...
        if cord.shape[1] > 0:
            ...
            new_cord[0] = int(new_cord[0] + padding[0][0])  # axis = y
            new_cord[1] = int(new_cord[1] + padding[1][0])  # axis = x
            new_cord = np.array(new_cord, dtype=np.int32)
            final_mask[new_cord[0], new_cord[1], i] = 1
...
def minimize_mask(bbox, mask, mini_shape):
    ...
    #             mini_mask[coo[0][0], coo[1][0], i + ii] = True
    #         else:
    #             mini_mask[:, :, i + ii] = False
    return mini_mask


def minimize_mask_2(bbox, mask, mini_shape):
