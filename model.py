...
from Mask_R_CNN_KERAS import my_utils as utils
...
def detection_targets_graph(proposals, gt_boxes, gt_masks, config):
    ...
    rois = tf.reshape(tf.tile(tf.expand_dims(proposals, 1), [1, 1, tf.shape(gt_boxes)[0]]), [-1, 4])
    boxes = tf.tile(gt_boxes, [tf.shape(proposals)[0], 1])
    # 2. Compute intersections
    roi_y1, roi_x1, roi_y2, roi_x2 = tf.split(rois, 4, axis=1)
    box_y1, box_x1, box_y2, box_x2, class_ids = tf.split(boxes[:, :5], 5, axis=1)
    ...
    class_mask_ids = roi_gt_boxes[:, 5:]

    return rois, roi_gt_boxes[:, 4], deltas, masks, class_mask_ids
...
class DetectionTargetLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        names = ["rois", "target_class_ids", "target_bbox", "target_mask", "target_maskclass_ids"]
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        def wrapper(rois, mrcnn_class, mrcnn_bbox, image_meta):
            ...
            # Reshape output
            # [batch, num_detections, (y1, x1, y2, x2, class_score)] in pixels
            return np.reshape(detections, [1, self.config.DETECTION_MAX_INSTANCES, 6])
...
def load_image_gt(dataset, config, image_id, augment=False, use_mini_mask=False):
    ...
    mask, class_ids, class_mask = dataset.load_mask(image_id)
    shape = image.shape
    ...
    buffer_mask = np.zeros([mask.shape[0], image.shape[0], image.shape[1], mask.shape[3]])
    for i in range(mask.shape[0]):
        buffer_mask[i, :, :, :] = utils.resize_keypoints(mask[i, :, :, :], image.shape[:2], scale, padding)
        # buffer_mask[i, :, :, :] = utils.resize_mask(mask[i, :, :, :], scale, padding)
    mask = buffer_mask
    fliped = False
    ...
    if augment:
        if random.randint(0, 1):
            ...
            buffer_mask = np.zeros_like(mask)
            for i in range(mask.shape[0]):
                buffer_mask[i] = np.fliplr(mask[i])
            fliped = True
            mask = buffer_mask
    ...
    for h_num in range(human_num):
        ...
        bbox[h_num, :4] = temp_bbox
        bbox[h_num, 4] = 1
        bbox[h_num, 5:] = class_mask[h_num, :]
...
def data_generator(dataset, config, shuffle=True, augment=False, random_rois=0,
    ...
    while True:
        try:
            ...
            # RPN Targets
            rpn_match, rpn_bbox = build_rpn_targets(image.shape, anchors, gt_boxes[:, :5], config)

            # Mask R-CNN Targets
            if random_rois:
                rpn_rois = generate_random_rois(image.shape, random_rois, gt_boxes[:, :5])
                if detection_targets:
                    ...
                    rois, mrcnn_class_ids, mrcnn_bbox, mrcnn_mask = \
                        build_detection_targets(rpn_rois, gt_boxes[:, :5], gt_masks, config)
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir):
        ...
        config: A Sub-class of the Config class, a setup of the of configuration
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            input_gt_boxes = KL.Input(shape=[None, 19], name="input_gt_boxes", dtype=tf.int32)
            ...
            for i in range(15):
                add_class.append(1)
            image_scale = K.cast(K.stack(add_class, axis=0), tf.float32)
        ...
        if mode == "training":
            ...
            _, _, _, active_class_ids = KL.Lambda(
                lambda x: parse_image_meta_graph(x), mask=[None, None, None, None])(input_image_meta)
            ...
            rois, target_class_ids, target_bbox, target_mask, target_maskclass_ids = DetectionTargetLayer(
                config, name="proposal_targets")([target_rois, gt_boxes, input_gt_masks])

            # Network Heads
            # TODO: verify that this handles zero padded ROIs
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_class_mask = fpn_classifier_graph(
            ...
            active_class_ids = KL.Lambda(lambda x: x * 1, name='active_class_ids')(active_class_ids)
            ...
            mask_class_loss = KL.Lambda(lambda x: mask_class_loss_graph(*x), name="mask_class_loss")(
                [target_maskclass_ids, mrcnn_class_mask, target_class_ids])
            ...
            outputs = [rpn_class_logits, rpn_class, rpn_bbox,
                       mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_mask, mrcnn_class_mask,
                       rpn_rois, output_rois, active_class_ids,
                       rpn_class_loss, rpn_bbox_loss, class_loss, mask_class_loss, bbox_loss, mask_loss]

            model = KM.Model(inputs, outputs, name='mask_rcnn')
        else:
            # Network Heads
            # Proposal classifier and BBox regressor heads
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_class_mask = \
            ...
            model = KM.Model([input_image, input_image_meta],
                             [detections, mrcnn_class, mrcnn_bbox, mrcnn_mask,
                              rpn_rois, rpn_class, rpn_bbox, mrcnn_class_mask],
                             name='mask_rcnn')

        # Add multi-GPU support.
        if config.GPU_COUNT > 1:
            from Mask_R_CNN_KERAS.parallel_model import ParallelModel
    ...
    def compile(self, learning_rate, momentum):
        ...
        loss_names = ["rpn_class_loss", "rpn_bbox_loss", "mrcnn_class_loss",
                      "mask_class_loss", "mrcnn_bbox_loss", "mrcnn_mask_loss"]
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        # Data generators
        train_generator = data_generator(train_dataset, self.config, shuffle=True, batch_size=self.config.BATCH_SIZE,
        ...
        fit_kwargs = {
            ...
            "workers": max(self.config.BATCH_SIZE // 2, 1),
    ...
    def run_graph(self, images, outputs, TEST_MODE=None, config=None):
        ...
        if TEST_MODE == "training":
            # Run inference
            molded_images, image_metas, windows = self.mold_inputs(images[0])
            target_rpn_match = images[2]
            target_rpn_bbox = images[3]
            gt_boxes = images[4]
            gt_masks = images[5]
            model_in = [molded_images, image_metas, target_rpn_match, target_rpn_bbox, gt_boxes, gt_masks]
            # if not config.USE_RPN_ROIS:
